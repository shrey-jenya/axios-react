...
const App = () => {
  ...
  const filterSearch = myData.filter((user) =>
    user.name.toLowerCase().includes(searchQuery.toLowerCase())
  );
  return (
    <div>
      <h1>PlaceHolder API - USERS</h1>
      <input type="search" value={searchQuery} onChange={handleSearch} placeholder='Search User...' />
      {isError && <h1>{isError}</h1>}
      {
        loading ? <h1>Loading...</h1> : (
          filterSearch.map(({ username, name, id, email, phone, website }) => (
